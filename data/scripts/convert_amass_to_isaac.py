...
os.makedirs("/tmp/smpl_zys", exist_ok=True)
TMP_SMPL_DIR = "/tmp/smpl_zys"
...
def axis_angle_yup_to_zup(axis_angle, rotate_angle=-90):
    ...
    Parameters:
    - axis_angle: (N, 3) numpy array of axis-angle rotations (Y-up)

    Returns:
    - (N, 3) numpy array of axis-angle rotations in Z-up
    """
    axis_angle = np.asarray(axis_angle)
    r = R.from_rotvec(axis_angle)

    # Define rotation to convert from Y-up to Z-up (rotate -90° around X)
    R_yup_to_zup = R.from_euler('x', rotate_angle, degrees=True)

    # Apply: new_rotation = R_align * original_rotation
    r_new = R_yup_to_zup * r  # Apply on the left
...
def main(
    ...
    for folder_name in folder_names:
        ...
        all_files_in_folder = [
            ...
            if (f.name != "shape.npz" and "stagei.npz" not in f.name and f.name != 'object_fit_all.npz')
        ]
        ...
        for filename in tqdm(files):
            try:
                ...
                if filename.suffix == ".npz" and "samp" not in str(filename):
                    ...
                    if yup2zup:
                        amass_pose = motion_data["poses"]
                        amass_trans = yup_to_zup(amass_trans)
                        amass_pose[:,:3] = axis_angle_yup_to_zup(amass_pose[:,:3], -90)
                    ...
                    if humanoid_type == "smplx":
                        ...
                        if "mocap_framerate" in motion_data:
                            ...
                            mocap_fr = motion_data["mocap_frame_rate"]
                        elif 'behave-30fps' in str(filename):
                            mocap_fr = 30
